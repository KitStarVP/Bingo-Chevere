...
class SalaJuego {
    ...
    createCardElement(card) {
        ...
        // Asegurar que autoMode esté definido
        ...
        cardDiv.innerHTML = `
            ...
            <div class="mini-controls">
                <div class="mini-toggle ${card.autoMode ? 'active' : ''}" onclick="salaJuego.toggleAutoMode('${cardId}')" title="${card.autoMode ? 'Modo Automático' : 'Modo Manual'}">
                    ${card.autoMode ? 'A' : 'M'}
                </div>
    ...
    toggleAutoMode(cardId) {
        const card = this.playerCards.find(c => c.id == cardId);
        if (!card) {
            console.log('Cartón no encontrado:', cardId);
            return;
        }
        ...
        // Si se activa modo automático, marcar números ya cantados
        if (card.autoMode) {
            this.autoMarkCard(card);
            this.showMessage('Modo automático activado');
        } else {
        ...
        console.log(`Cartón ${cardId} cambiado a modo:`, card.autoMode ? 'Automático' : 'Manual');
    }
